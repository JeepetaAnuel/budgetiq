    ...
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/85 backdrop-blur-lg shadow-sm border-b border-slate-200' : 'bg-transparent'}`}>
        <div className="max-w-6xl mx-auto flex items-center justify-between h-16 px-6">
          <a href="/" className="flex items-center gap-2 text-xl font-extrabold text-slate-900">
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <button onClick={() => navigate('/login')} className="flex items-center gap-1.5 text-sm font-medium text-slate-500 hover:text-slate-900 transition-colors">
              <LogIn size={14} /> Iniciar sesión
            </button>
            <button onClick={() => navigate('/dashboard')} className="text-sm font-semibold px-5 py-2.5 bg-slate-900 text-white rounded-full hover:bg-emerald-600 transition-all shadow-lg shadow-slate-900/20 hover:shadow-emerald-600/20">
              Ir a la App
            </button>
          </nav>
          <button onClick={() => setMenuOpen(v => !v)} className="md:hidden p-2 text-slate-700">
            {menuOpen ? <X size={22} /> : <Menu size={22} />}
          </button>
        </div>
        {menuOpen && (
          <div className="md:hidden bg-white border-b border-slate-200 px-6 py-4 space-y-3">
            <a href="#features" onClick={() => setMenuOpen(false)} className="block text-sm font-medium text-slate-600 py-1">Funciones</a>
            <a href="#how" onClick={() => setMenuOpen(false)} className="block text-sm font-medium text-slate-600 py-1">Cómo funciona</a>
            <a href="#faq" onClick={() => setMenuOpen(false)} className="block text-sm font-medium text-slate-600 py-1">FAQ</a>
            <button onClick={() => { setMenuOpen(false); navigate('/login') }} className="w-full text-sm font-medium text-slate-600 py-1">Iniciar sesión</button>
        ...
        <div className="max-w-6xl mx-auto px-6 md:grid md:grid-cols-2 md:gap-12 items-center">
          <div className="anim-fade" style={{animationDelay: '0.1s'}}>
            ...
            <div className="flex flex-wrap gap-3">
              <button onClick={() => navigate('/dashboard')} className="inline-flex items-center gap-2 px-7 py-3.5 bg-slate-900 text-white font-semibold rounded-full text-sm hover:bg-emerald-600 transition-all shadow-xl shadow-slate-900/20 hover:shadow-emerald-600/20">
          ...
          <div className="mt-10 md:mt-0 anim-fade" style={{animationDelay: '0.3s'}}>
            <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 p-6 max-w-sm mx-auto">
      ...
      <section className="py-16 md:py-20 bg-slate-50/50">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {CATEGORIES.map(c => (
              <div key={c.name} className="bg-white border border-slate-200 rounded-xl p-4 text-center hover:shadow-md hover:-translate-y-0.5 transition-all cursor-default">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {FEATURES.map(f => (
              <div key={f.step} className="text-center">
                <div className="w-14 h-14 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center text-xl font-extrabold mx-auto mb-5">{f.step}</div>
      ...
      <section id="features" className="py-16 md:py-20 bg-slate-50/50">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="bg-gradient-to-br from-emerald-600 to-emerald-700 rounded-2xl p-8 md:p-10 text-white text-center">
            ...
            <button onClick={() => navigate('/dashboard')} className="inline-flex items-center gap-2 px-8 py-3.5 bg-white text-emerald-700 font-bold rounded-full text-sm hover:bg-emerald-50 transition-all shadow-xl">
              Ir a la App <ArrowRight size={16} />
            </button>
          </div>
        </div>
      ...
      <section className="py-16 md:py-20">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {TESTIMONIALS.map(t => (
              <div key={t.name} className="bg-slate-50/50 border border-slate-200 rounded-2xl p-6 text-left">
                <div className="text-amber-400 text-sm mb-3">★★★★★</div>
                <p className="text-sm text-slate-600 italic mb-5">"{t.text}"</p>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-400 flex items-center justify-center text-white text-xs font-bold">{t.initials}</div>
          ...
          <div className="text-left space-y-2">
            {FAQS.map((f, i) => (
              <div key={i} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                <button onClick={() => setFaqOpen(faqOpen === String(i) ? null : String(i))} className="w-full flex justify-between items-center px-5 py-4 text-sm font-semibold text-slate-900 text-left">
                  {f.q}
                  <span className={`text-slate-400 text-xl transition-transform ${faqOpen === String(i) ? 'rotate-45' : ''}`}>+</span>
                </button>
                {faqOpen === String(i) && <div className="px-5 pb-4 text-sm text-slate-500 leading-relaxed">{f.a}</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      <footer className="bg-slate-900 text-slate-400 py-12">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid md:grid-cols-4 gap-8 pb-8 border-b border-slate-800">
            ...
            <div><h5 className="text-xs font-bold text-white uppercase tracking-wider mb-4">Producto</h5><ul className="space-y-2 text-xs"><li><a href="#" className="hover:text-white transition-colors">Funciones</a></li><li><a href="#" className="hover:text-white transition-colors">Precios</a></li><li><a href="#" className="hover:text-white transition-colors">Modo IA</a></li><li><a href="#" className="hover:text-white transition-colors">Modo Manual</a></li></ul></div>
            <div><h5 className="text-xs font-bold text-white uppercase tracking-wider mb-4">Compañía</h5><ul className="space-y-2 text-xs"><li><a href="#" className="hover:text-white transition-colors">Blog</a></li><li><a href="#" className="hover:text-white transition-colors">Contacto</a></li><li><a href="#" className="hover:text-white transition-colors">Privacidad</a></li><li><a href="#" className="hover:text-white transition-colors">Términos</a></li></ul></div>
            <div><h5 className="text-xs font-bold text-white uppercase tracking-wider mb-4">Descarga</h5><div className="flex flex-col gap-2 text-xs"><span className="text-slate-500">App Web (PWA)</span><span className="text-slate-500">iOS · Android · Web</span><button onClick={() => navigate('/dashboard')} className="mt-2 text-emerald-400 font-semibold hover:text-emerald-300 transition-colors">Ir a la App →</button></div></div>
          </div>
